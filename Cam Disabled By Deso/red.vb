...
#Region " Initialization "
    ...
    Public Shared Sub DrawRoundRectangle(ByVal g As Graphics, ByVal pen As Pen, ByVal Rect As Rectangle, ByVal cornerSize As Size)
        Dim points(7) As Point
        Dim rX As Single = Rect.Location.X
        Dim rY As Single = Rect.Location.Y
        Dim rW As Single = Rect.Width
        Dim rH As Single = Rect.Height
        Dim cW As Single = cornerSize.Width
        Dim cH As Single = cornerSize.Height

        points(0) = New Point(rX + cW / 2, rY)
        points(1) = New Point(rX + rW - cW / 2, rY)
        points(2) = New Point(rX + rW, rY + cH / 2)
        points(3) = New Point(rX + rW, rY + rH - cH / 2)
        points(4) = New Point(rX + rW - cW / 2, rY + rH)
        points(5) = New Point(rX + cW / 2, rY + rH)
        points(6) = New Point(rX, rY + rH - cH / 2)
        points(7) = New Point(rX, rY + cH / 2)
    ...
    Public Shared Sub FillRoundRectangle(ByVal g As Graphics, ByVal brush As Brush, ByVal Rect As Rectangle, ByVal cornerSize As Size)
        Dim points(7) As Point
        Dim rX As Single = Rect.Location.X
        Dim rY As Single = Rect.Location.Y
        Dim rW As Single = Rect.Width
...
Public Class TestThemeEditorForm
    Inherits ThemeContainer154
    Dim pclose As New PictureBox
    Dim pminimize As New PictureBox
    Sub New()
        Sizable = True
        ...
        Me.Controls.Add(pclose)
        AddHandler pclose.Click, AddressOf pclose_Click
        pminimize.Size = New Size(16, 16)
        pminimize.BackColor = Color.Transparent
        Me.Controls.Add(pminimize)
        AddHandler pminimize.Click, AddressOf pminimize_Click
    End Sub
    Private Sub pclose_Click(ByVal sender As Object, ByVal e As EventArgs)
        End
    End Sub
    Private Sub pminimize_Click(ByVal sender As Object, ByVal e As EventArgs)
    ...
    Protected Overrides Sub PaintHook()
        ...
        pminimize.Location = New Point(FindForm.Width - 40, 6)
        pclose.Location = New Point(FindForm.Width - 22, 6)
        G.Clear(Color.FromArgb(0, 0, 0))
        DrawBorders(New Pen(Color.FromArgb(255, 15, 15)))
        G.DrawString("r", New Font("Marlett", 7), New SolidBrush(Color.FromArgb(255, 0, 0)), New Point(FindForm.Width - 20, 12))
        G.DrawString("0", New Font("Marlett", 7), New SolidBrush(Color.FromArgb(255, 0, 0)), New Point(FindForm.Width - 38, 9))
        DrawText(New SolidBrush(Color.FromArgb(255, 0, 0)), HorizontalAlignment.Center, 0, 5)
        G.FillRectangle(New SolidBrush(Color.FromArgb(0, 0, 0)), New Rectangle(7, 35, Width - 15, Height - 42))
        G.DrawIcon(FindForm.Icon, New Rectangle(7, 8, 16, 16))
    End Sub
